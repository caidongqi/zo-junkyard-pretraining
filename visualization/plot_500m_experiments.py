...
def find_csv_files():
    ...
    # 查找所有500M的CSV文件
    pattern = "logs/parallel_sweep_*/experiments/*500M*/logs/*/csv_logs_*/*500M*.csv"
    all_files = glob.glob(pattern)
    
    if not all_files:
        print(f"未找到500M实验的CSV文件 (pattern: {pattern})")
        return csv_files
    ...
    for csv_file in all_files:
        ...
        for part in parts:
            if '500M' in part and ('FO' in part or 'Instruct' in part):
                exp_name = part
                break
...
def plot_loss_curves(csv_files, output_file):
    ...
    ax.set_title('Loss Curves: All 500M Model Experiments', fontsize=16, fontweight='bold')
...
def main():
    ...
    plot_loss_curves(
        csv_files,
        output_dir / 'loss_curves_all_500m.png'
    )
